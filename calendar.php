...
<section class="vcf-section">
    <div class="container">
        <h1 class="vcf-section-title">Add to my calendar</h1>
        <p class="vcf-section-desc"><?= htmlspecialchars($title) ?> — <?= date('l, F j, Y', $startTs) ?> at <?= date('g:i A', $startTs) ?></p>
        <p class="text-white-50 mb-3"><?= htmlspecialchars($location) ?></p>
        <div class="d-flex flex-wrap gap-2">
            <a href="<?= htmlspecialchars($gUrl) ?>" class="vcf-btn-cta" target="_blank" rel="noopener noreferrer">Open in Google Calendar</a>
            <a href="calendar.php?id=<?= $id ?>&amp;format=ics" class="btn btn-outline-secondary">Download .ics (Apple / Outlook)</a>
            <a href="<?= isset($base) ? $base : '' ?>/index.php#tournaments" class="btn btn-outline-secondary">Back to schedule</a>
        </div>
    </div>
</section>
