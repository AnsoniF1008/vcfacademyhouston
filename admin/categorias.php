...
// Cache SHOW COLUMNS result within the request (avoids duplicate queries)
static $hasHorariosTable = null;
if ($hasHorariosTable === null) {
    try {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrf_verify()) {
        ...
        $stName = $pdo->prepare("SELECT nombre FROM categorias WHERE id = ?");
        $stName->execute([$id]);
        $delName = $stName->fetchColumn() ?: $id;
        $pdo->prepare("DELETE FROM categorias WHERE id = ?")->execute([$id]);
        admin_log('categorias.delete', 'Deleted category "' . $delName . '" (id ' . $id . ')');
        $message = 'Category deleted.'; $messageType = 'success';
    } else {
        $id     = isset($_POST['id']) ? (int) $_POST['id'] : 0;
        $nombre = mb_substr(trim($_POST['nombre'] ?? ''), 0, 100);

        if ($nombre === '') {
            $message = 'Category name is required.'; $messageType = 'danger';
        } else {
            $horariosLegacy = mb_substr(trim($_POST['horarios_entrenamiento'] ?? ''), 0, 255);
            if ($id > 0) {
                if ($hasHorariosTable) {
                    $pdo->prepare("UPDATE categorias SET nombre = ? WHERE id = ?")->execute([$nombre, $id]);
                } else {
                    $pdo->prepare("UPDATE categorias SET nombre = ?, horarios_entrenamiento = ? WHERE id = ?")
                        ->execute([$nombre, $horariosLegacy ?: null, $id]);
                }
                $catId = $id;
                admin_log('categorias.update', 'Updated category "' . $nombre . '" (id ' . $id . ')');
                $message = 'Category updated.';
            } else {
                $pdo->prepare("INSERT INTO categorias (nombre, horarios_entrenamiento) VALUES (?, ?)")
                    ->execute([$nombre, $horariosLegacy ?: null]);
                $catId = (int) $pdo->lastInsertId();
                admin_log('categorias.create', 'Created category "' . $nombre . '"');
                ...
                <div class="card-body">
                    ...
                    <form method="post" action="" id="formCategoria">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control bg-dark text-white border-secondary" name="nombre" required maxlength="100" placeholder="U6, U8, U10, B13" value="<?= htmlspecialchars($editing['nombre'] ?? '') ?>">
                        </div>
                        <?php if (!$hasHorariosTable): ?>
                        <div class="mb-3">
                            <label class="form-label text-white small">Training schedule (e.g. Tue/Thu 4:00 PM)</label>
                            <input type="text" class="form-control bg-dark text-white border-secondary" name="horarios_entrenamiento" maxlength="255" placeholder="Mon/Wed 4:30 PM - 5:30 PM" value="<?= htmlspecialchars($editing['horarios_entrenamiento'] ?? '') ?>">
                        </div>
                        <?php else: ?>
                        <div class="mb-3">
                    ...
                    <?php else: ?>
                        <div class="table-responsive">
                            <table class="table table-dark table-sm">
                                <thead><tr><th>Category</th><th>Schedule</th><th></th></tr></thead>
                                <tbody>
...
(function() {
    ...
    var dayOpts = <?= json_encode(array_map(function($d) use ($dayNames) {
