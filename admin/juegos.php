...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrf_verify()) {
        ...
        if (!$game || $game['goles_vcf'] === null) {
            $message = 'Game not found or has no score.'; $messageType = 'danger';
        } else {
            ...
            if ($totalGoles !== $expected) {
                $message = "Total goals must equal VCF score ($expected). You entered $totalGoles."; $messageType = 'danger';
            } else {
        ...
        $pdo->prepare("DELETE FROM juegos WHERE id = ? AND torneo_id = ?")->execute([$id, $torneo_id]);
        admin_log('juegos.delete', 'Deleted game ' . $id . ' (torneo ' . $torneo_id . ')');
        $message = 'Game deleted.'; $messageType = 'success';
    } elseif (isset($_POST['delete_id']) && !admin_can('juegos_delete')) {
        $message = 'You do not have permission to delete games.'; $messageType = 'danger';
        ...
        if ($id > 0) {
            ...
            $pdo->prepare("UPDATE juegos SET estado=?, goles_vcf=?, goles_rival=? WHERE id=? AND torneo_id=?")
                ->execute([$estado, $goles_vcf, $goles_rival, $id, $torneo_id]);
            admin_log('juegos.live_score', 'Updated score game ' . $id . ': ' . ($goles_vcf ?? '') . '-' . ($goles_rival ?? ''));
            $message = 'Score updated.'; $messageType = 'success';
        }
    } else {
        if (!admin_can('juegos')) {
            $message = 'You can only update live score.'; $messageType = 'danger';
        } else {
            $id                = isset($_POST['id']) ? (int) $_POST['id'] : 0;
            $fecha             = trim($_POST['fecha'] ?? '');
            $horaRaw           = trim($_POST['hora']  ?? '');
            $hora              = $horaRaw !== '' ? $horaRaw . (substr_count($horaRaw, ':') === 1 ? ':00' : '') : null;
            $rival             = mb_substr(trim($_POST['rival']             ?? ''), 0, 255) ?: null;
            $cancha            = mb_substr(trim($_POST['cancha']            ?? ''), 0, 255) ?: null;
            $sede_id           = isset($_POST['sede_id']) && $_POST['sede_id'] !== '' ? (int) $_POST['sede_id'] : null;
            $ubicacion_mapa_url = mb_substr(trim($_POST['ubicacion_mapa_url'] ?? ''), 0, 2083) ?: null;
            $estado            = in_array($_POST['estado'] ?? '', ['finalizado','live'], true) ? $_POST['estado'] : 'proximo';
            $categoria         = mb_substr(trim($_POST['categoria']         ?? ''), 0, 100) ?: null;
            $goles_vcf         = isset($_POST['goles_vcf'])   && $_POST['goles_vcf']   !== '' ? (int) $_POST['goles_vcf']   : null;
            $goles_rival       = isset($_POST['goles_rival']) && $_POST['goles_rival'] !== '' ? (int) $_POST['goles_rival'] : null;
            $rival_logo_url    = mb_substr(trim($_POST['rival_logo_url']    ?? ''), 0, 2083) ?: null;

            if ($fecha === '') {
                $message = 'Date is required.'; $messageType = 'danger';
            } else {
                if ($id > 0) {
                    $pdo->prepare("UPDATE juegos SET fecha=?, hora=?, rival=?, cancha=?, sede_id=?, ubicacion_mapa_url=?, estado=?, categoria=?, goles_vcf=?, goles_rival=?, rival_logo_url=? WHERE id=? AND torneo_id=?")
                        ->execute([$fecha, $hora, $rival, $cancha, $sede_id, $ubicacion_mapa_url, $estado, $categoria, $goles_vcf, $goles_rival, $rival_logo_url, $id, $torneo_id]);
                    admin_log('juegos.update', 'Updated game ' . $id . ' vs ' . ($rival ?: 'TBD'));
                    $message = 'Game updated.';
                } else {
                    $pdo->prepare("INSERT INTO juegos (torneo_id, fecha, hora, rival, cancha, sede_id, ubicacion_mapa_url, estado, categoria, goles_vcf, goles_rival, rival_logo_url) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)")
                        ->execute([$torneo_id, $fecha, $hora, $rival, $cancha, $sede_id, $ubicacion_mapa_url, $estado, $categoria, $goles_vcf, $goles_rival, $rival_logo_url]);
...
if (isset($_GET['edit'])) {
    ...
    foreach ($juegos as $j) {
        if ((int) $j['id'] === $editId) { $editing = $j; break; }
    }
            ...
            <div class="card bg-dark border border-secondary rounded-3">
                <div class="card-body">
                    ...
                    <form method="post" action="?torneo_id=<?= $torneo_id ?>">
                        ...
                        <div class="mb-2"><label class="form-label text-white small">Date</label>
                            <input type="date" class="form-control bg-dark text-white border-secondary" name="fecha" required value="<?= htmlspecialchars($editing['fecha'] ?? '') ?>"></div>
                        <div class="mb-2"><label class="form-label text-white small">Time (optional)</label>
                            <input type="time" class="form-control bg-dark text-white border-secondary" name="hora" value="<?= isset($editing['hora']) && $editing['hora'] !== null ? substr($editing['hora'], 0, 5) : '' ?>"></div>
                        <div class="mb-2"><label class="form-label text-white small">Opponent (rival)</label>
                            <input type="text" class="form-control bg-dark text-white border-secondary" name="rival" maxlength="255" placeholder="e.g. North Shore FC" value="<?= htmlspecialchars($editing['rival'] ?? '') ?>"></div>
                        <div class="mb-2"><label class="form-label text-white small">Rival logo (URL, optional)</label>
                            <input type="url" class="form-control bg-dark text-white border-secondary" name="rival_logo_url" placeholder="https://..." value="<?= htmlspecialchars($editing['rival_logo_url'] ?? '') ?>"></div>
                        <div class="mb-2"><label class="form-label text-white small">Field / Cancha</label>
                            <input type="text" class="form-control bg-dark text-white border-secondary" name="cancha" maxlength="255" placeholder="e.g. Field 7 or Meyer Park - Field 5" value="<?= htmlspecialchars($editing['cancha'] ?? '') ?>"></div>
                        <div class="mb-2"><label class="form-label text-white small">Venue (sede, optional)</label>
                        ...
                        <div class="mb-2"><label class="form-label text-white small">Map URL (optional)</label>
                            <input type="url" class="form-control bg-dark text-white border-secondary" name="ubicacion_mapa_url" placeholder="https://maps.google.com/..." value="<?= htmlspecialchars($editing['ubicacion_mapa_url'] ?? '') ?>"></div>
                        <div class="mb-2"><label class="form-label text-white small">Category (e.g. U10)</label>
                            <input type="text" class="form-control bg-dark text-white border-secondary" name="categoria" maxlength="100" placeholder="U8, U10, U12" value="<?= htmlspecialchars($editing['categoria'] ?? '') ?>"></div>
                        <div class="mb-2"><label class="form-label text-white small">Score: VCF</label>
                            <input type="number" min="0" class="form-control bg-dark text-white border-secondary" name="goles_vcf" placeholder="—" value="<?= isset($editing['goles_vcf']) && $editing['goles_vcf'] !== null ? (int) $editing['goles_vcf'] : '' ?>"></div>
                        <div class="mb-2"><label class="form-label text-white small">Score: Opponent</label>
                            <input type="number" min="0" class="form-control bg-dark text-white border-secondary" name="goles_rival" placeholder="—" value="<?= isset($editing['goles_rival']) && $editing['goles_rival'] !== null ? (int) $editing['goles_rival'] : '' ?>"></div>
                        <div class="mb-3"><label class="form-label text-white small">Status</label>
                            ...
                            <small class="text-muted">On the list, status is shown by date.</small></div>
                        <button type="submit" class="btn btn-primary" style="background:#FF6600;border:none;"><?= $editing ? 'Update' : 'Add' ?></button>
                        <?php if ($editing): ?><a href="juegos.php?torneo_id=<?= $torneo_id ?>" class="btn btn-secondary">Cancel</a><?php endif; ?>
                    </form>
                    ...
                    <p class="small text-muted">Assign goals and assists. Total goals must equal VCF score (<?= (int) $editing['goles_vcf'] ?>).</p>
                    ...
                    <?php else: ?>
                    <p class="small text-warning mb-0">No roster found for category "<?= htmlspecialchars($editing['categoria'] ?? '') ?>".</p>
                    <?php endif; ?>
                    <?php endif; ?>
                </div>
            </div>
        ...
        <div class="col-lg-7">
            <div class="card bg-dark border border-secondary rounded-3">
                <div class="card-body">
                    ...
                    <?php $houstonNow = new DateTime('now', new DateTimeZone('America/Chicago')); ?>
                    <p class="text-muted small mb-2">Reference time (Houston): <strong><?= $houstonNow->format('l, M j, Y g:i A T') ?></strong></p>
                    ...
                    <?php else: ?>
                        <div class="table-responsive">
                            <table class="table table-dark table-sm">
                                <thead>
                                    <tr><th>Date</th><th>Time</th><th>Opponent</th><th>Score</th><th>Location</th><th>Status</th><th></th></tr>
                                </thead>
                                <tbody>
                                    <?php
                                    ...
                                    foreach ($juegos as $j):
                                        ...
                                        if ($isPast)         { $statusLabel = 'Finished'; $statusClass = 'text-secondary'; }
                                        elseif ($isToday)    { $statusLabel = 'Live';     $statusClass = 'text-warning'; }
                                        else                 { $statusLabel = 'Upcoming'; $statusClass = 'text-info'; }
                                    ?>
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <?php if (admin_can('juegos_delete')): ?>
